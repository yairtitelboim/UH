...
const MapComponent = () => {
    ...
    const demographicsMessage = {
      ...
      quickActions: [
        ...
        },
        {
          ...
          followUpSuggestions: [
            {
              text: "Show Smart Grid expansion plans",
              prompt: "SHOW_SMART_GRID_EXPANSION"
            },
            {
              text: "View renewable energy targets",
              prompt: "SHOW_RENEWABLE_TARGETS"
            },
            {
              text: "Analyze projected power demand",
              prompt: "SHOW_POWER_DEMAND_FORECAST"
            },
            {
              text: "Infrastructure upgrade timeline",
              prompt: "SHOW_UPGRADE_TIMELINE"
            }
          ]
        }
      ]
  ...
  // Add this helper function
  const getGEOIDFeatures = () => {
    if (!map.current) return [];
    
    return brickellGEOIDs.map(geoid => {
        const source = map.current.getSource(`geoid-source-${geoid}`);
        const layer = map.current.getLayer(`hatched-area-${geoid}`);
