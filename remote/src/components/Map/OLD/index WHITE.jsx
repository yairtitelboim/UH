  ...
  useEffect(() => {
    ...
    function initializeLayersAndAnimation() {
        ...
        function animateParticles() {
            ...
            // Query rendered features for buildings and roads
            ...
            // Generate green particles first
            greenBuildingsData.forEach(({ building, center: buildingCenter }) => {
                const numGreenParticles = 50;
                const radius = 0.0005;
                
                for (let i = 0; i < numGreenParticles; i++) {
                ...
                // For each building, find nearby road segments
                const nearbyRoads = roads.filter(road => {
                    ...
                    // Check if any part of the road is near the building
                    return roadCoords.some(coord => {
                        ...
                        return distToBuilding < 0.002; // Increased radius to catch more roads
                    });
                });

                // Generate white particles along these nearby roads
                nearbyRoads.forEach(road => {
                    ...
                    // Generate particles along each road segment
                    for (let i = 0; i < roadCoords.length - 1; i++) {
                        const start = roadCoords[i];
                        const end = roadCoords[i + 1];
                        ...
                        const particlesPerSegment = Math.ceil(segmentLength * 10000);

                        for (let j = 0; j < particlesPerSegment; j++) {
                            const timeOffset = ((currentTime / 1000) + (j / particlesPerSegment)) % 1;
                            const position = timeOffset;

                            whiteRoadParticles.features.push({
                                type: 'Feature',
                                properties: {
                                    particleSize: 2.4 + Math.random() * 0.6,
