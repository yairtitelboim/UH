...
const COGENT_PARTICLE_COLOR = 'rgba(135, 206, 250, 0.8)'; // Light blue
        ...
        features: targetBuildings.map(building => {
            ...
            return {
                ...
                properties: {
                    ...
                    duration: Math.random() * 1.5 + 0.5 // Random duration between 0.5 and 2 seconds
                },
                geometry: {
                    type: 'LineString',
...
const MapComponent = (props) => {
  ...
  const cogentAnimationFrame = useRef(null);

  // At the component level, add these refs to store the buildings
  const greenBuildingRef = useRef(null);
  const nearbyBuildingsRef = useRef([]);
    ...
    try {
        if (!cogentActive) {
            ...
            if (greenBuilding) {
                ...
                // Add Cogent particle source if it doesn't exist
                if (!map.current.getSource('cogent-flow')) {
                    map.current.addSource('cogent-flow', {
                        type: 'geojson',
                        data: {
                            type: 'FeatureCollection',
                            features: []
                    ...
                    map.current.addLayer({
                        ...
                        paint: {
                            'circle-radius': 2.5,
                            'circle-color': COGENT_PARTICLE_COLOR,
                            'circle-blur': 1,
                            'circle-opacity': 0.8
                        }
                    });
                ...
                // Create particle animation function that doesn't interfere with existing animation
                const animateCogentParticles = () => {
                    if (!map.current || !map.current.loaded()) {
                        cogentAnimationFrame.current = requestAnimationFrame(animateCogentParticles);
                        return;
                    }
                    ...
                    nearbyBuildings.forEach(building => {
                        ...
                        for (let i = 0; i < 5; i++) {
                            const progress = (timestamp * 0.5 + i * 0.2) % 1;
                            
                            const lng = greenCoords[0] + (targetCoords[0] - greenCoords[0]) * progress;
                            const lat = greenCoords[1] + (targetCoords[1] - greenCoords[1]) * progress;
                    ...
                    if (map.current && map.current.getSource('cogent-flow')) {
                        map.current.getSource('cogent-flow').setData({
                            type: 'FeatureCollection',
                            features: particles
                        });
                    }

                    // Only continue animation if still in Cogent mode
                    if (cogentActive) {
                        cogentAnimationFrame.current = requestAnimationFrame(animateCogentParticles);
                    }
                };

                // Start the Cogent animation without stopping the existing animation
                cogentAnimationFrame.current = requestAnimationFrame(animateCogentParticles);

                // Add after your existing particle sources/layers setup
                map.current.addSource('cogent-road-particles', {
                    type: 'geojson',
                    data: {
                        type: 'FeatureCollection',
                        features: []
                ...
                // Add the blue Cogent particles layer
                map.current.addLayer({
                    'id': 'cogent-road-particles',
                    'type': 'circle',
                    'source': 'cogent-road-particles',
                    'paint': {
                        'circle-radius': [
                            'interpolate',
                            ['linear'],
                            ['zoom'],
                            8, ['*', ['get', 'particleSize'], 1.4],
                            10, ['*', ['get', 'particleSize'], 1.3],
                            12, ['*', ['get', 'particleSize'], 1.2],
                            14, ['*', ['get', 'particleSize'], 1.1],
                            16, ['*', ['get', 'particleSize'], 1.0]
                        ],
                        'circle-color': 'rgba(0, 191, 255, 0.9)', // Bright blue
                    ...
                    'layout': {
                        'visibility': 'none' // Hidden by default, shown when Cogent active
                    }
                });

                // Inside your handleCogentClick function, add:
                if (greenBuilding) {
                    // Store references
                    greenBuildingRef.current = greenBuilding;
                    nearbyBuildingsRef.current = nearbyBuildings;

                    // Show Cogent particles
                    if (map.current.getLayer('cogent-road-particles')) {
                        map.current.setLayoutProperty('cogent-road-particles', 'visibility', 'visible');
                    }
                } else {
                    // Clear references
                    greenBuildingRef.current = null;
                    nearbyBuildingsRef.current = [];
                    
                    // Hide Cogent particles
                    if (map.current.getLayer('cogent-road-particles')) {
                        map.current.setLayoutProperty('cogent-road-particles', 'visibility', 'none');
                    }
                }

                // In your existing animation function, add Cogent particle animation:
                function animateParticles() {
                    // ... existing animation code ...

                    if (cogentActive && greenBuildingRef.current) {
                        const cogentParticles = {
                            type: 'FeatureCollection',
                            features: []
                        };

                        // Get roads between green building and highlighted buildings
                        nearbyBuildingsRef.current.forEach(building => {
                            const roadPath = map.current.queryRenderedFeatures({
                            ...
                            roadPath.forEach(road => {
                                ...
                                for (let i = 0; i < coords.length - 1; i++) {
                                    const particleCount = Math.floor(20 * Math.random()) + 10;
                                    
                                    for (let p = 0; p < particleCount; p++) {
                                        const progress = ((Date.now() * 0.0001) + (p * 0.1)) % 1;
                                        
                                        const pos = [
                                            coords[i][0] + (coords[i+1][0] - coords[i][0]) * progress,
                                            coords[i][1] + (coords[i+1][1] - coords[i][1]) * progress
                                        ];

                                        cogentParticles.features.push({
                                            type: 'Feature',
                                            properties: {
                                                particleSize: 2 + Math.random(),
                        ...
                        // Update Cogent particles
                        if (map.current.getSource('cogent-road-particles')) {
                            map.current.getSource('cogent-road-particles').setData(cogentParticles);
                        }
                    }
  ...
  useEffect(() => {
    ...
    return () => {
        ...
        if (cogentAnimationFrame.current) {
            cancelAnimationFrame(cogentAnimationFrame.current);
            cogentAnimationFrame.current = null;
        }
        if (map.current) {
            map.current.remove();
        }
    ...
    map.current.on('click', '3d-buildings', (e) => {
      if (e.features.length > 0) {
        ...
        if (state && state.isGreen) {
            // Increase heatmap opacity for green building visualization
            [
                'cooling-heat',
                'building-distance-heatmap'
            ].forEach(layer => {
                if (map.current.getLayer(layer)) {
                    map.current.setPaintProperty(layer, 'heatmap-opacity', [
                        'interpolate',
                        ['linear'],
                        ['zoom'],
                        14, 0.19,  // Increased from default
                        16, 0.15   // Increased from default
                    ]);
                }
            });
        } else {
            // Reset heatmap opacity to default values
            [
                'cooling-heat',
                'building-distance-heatmap'
            ].forEach(layer => {
                if (map.current.getLayer(layer)) {
                    map.current.setPaintProperty(layer, 'heatmap-opacity', [
                        ...
                        ['linear'],
                        ['zoom'],
                        14, 0.2,  // Default value
                        16, 0.1   // Default value
                    ]);
                }
            });
        }
        
        // Only show the card if NOT in Cogent mode
        if (!cogentActive) {
