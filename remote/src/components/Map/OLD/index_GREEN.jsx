                ...
                buildingCoords.forEach((coord, index, array) => {
                    ...
                    for (let i = 0; i < particlesPerEdge; i++) {
                        ...
                        for (let layer = 0; layer < 5; layer++) {
                            ...
                            // Add random dark particles
                            const isDarkParticle = Math.random() < 0.1;
                            const brightness = isDarkParticle ? 
                                80 + Math.random() * 40 : // Dark particles
                                200 + Math.random() * 55;  // Bright particles
                            
                            const layerFade = Math.exp(-layer * 0.5); // Exponential falloff
                            const randomOffset = (Math.random() - 0.5) * 0.2; // Add some randomness to fade
                ...
                roads.forEach(road => {
                    ...
                    for (let idx = 0; idx < roadCoords.length - 1; idx++) {
                        ...
                        if (distToBuilding < 0.006 * totalScale) {
                            ...
                            // Increased particle density near buildings
                            ...
                            for (let i = 0; i < particlesPerRoadSegment; i++) {
                                ...
                                // Add random dark particles to roads too
                                const isDarkParticle = Math.random() < 0.1;
                                const baseGreen = isDarkParticle ? 100 : 220;
                                const greenValue = Math.floor(baseGreen * enhancedFade);
                                const redValue = Math.floor((isDarkParticle ? 40 : 80) * enhancedFade);
                                
                                // Add random variation to position and opacity
                                ...
                                greenBuildingParticles.features.push({
                                    type: 'Feature',
                                    properties: {
                                        particleSize: (isDarkParticle ? 0.9 : 1.2) * (enhancedFade + 0.1),
                                        opacity: Math.max(0.1, Math.min(0.9, (0.9 * enhancedFade) + opacityVariation)),
                                        color: `rgba(${redValue}, ${greenValue}, ${redValue}, ${Math.pow(enhancedFade, 1.2)})`
                                    },
                                    geometry: {
                                        type: 'Point',
                                        coordinates: [
