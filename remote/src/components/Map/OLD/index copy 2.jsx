...
const MapComponent = (props) => {
  ...
  const handleCogentClick = useCallback(() => {
    ...
    try {
        if (!cogentActive) {
            ...
            // Hide specific visualization layers but keep particles
            [
            ...
            // Keep particle layers visible but adjust their appearance for Cogent mode
            [
                'data-flow-particles',
                'tcp-flow-particles',
                'tcp2-flow-particles',
                'tcp3-flow-particles',
                'green-building-particles',
                'white-road-particles'
            ].forEach(layer => {
                if (map.current.getLayer(layer)) {
                    map.current.setPaintProperty(layer, 'circle-opacity', 0.4);
                    map.current.setPaintProperty(layer, 'circle-color', '#1a1a1a');
                }
            });

            // Rest of Cogent mode activation code...
        } else {
            // Exiting Cogent mode
            ...
            // Reset particle layers to original appearance
            [
                'data-flow-particles',
                'tcp-flow-particles',
                'tcp2-flow-particles',
                ...
                if (map.current.getLayer(layer)) {
                    map.current.setPaintProperty(layer, 'circle-opacity', 0.8);
                    map.current.setPaintProperty(layer, 'circle-color', '#ffffff');
                }
            });

            // Show all visualization layers again
            [
                'cooling-heat',
                'building-distance-heatmap'
            ].forEach(layer => {
  ...
  useEffect(() => {
    ...
    // Existing building click handler
    ...
    map.current.on('click', (e) => {
        // Only proceed if we didn't click a building
        const features = map.current.queryRenderedFeatures(e.point, {
            layers: ['3d-buildings']
        });
        
        if (features.length === 0) {
            // Reset heatmap opacity to default low values
            [
                'cooling-heat',
                'building-distance-heatmap'
            ].forEach(layer => {
                if (map.current.getLayer(layer)) {
                    map.current.setPaintProperty(layer, 'heatmap-opacity', [
                        ...
                        ['linear'],
                        ['zoom'],
                        14, 0.2,  // Default low value
                        16, 0.1   // Default low value
                    ]);
                }
            });
            
            // Clear selected building and floating card if they exist
            if (currentRoot.current) {
                currentRoot.current.unmount();
                currentRoot.current = null;
            }
  ...
  }, [map.current]); // Add map.current as dependency
  ...
  const createGreenLines = useCallback(() => {
      ...
      // Create connections between green buildings
      for (let i = 0; i < greenBuildings.length; i++) {
          for (let j = i + 1; j < greenBuildings.length; j++) {
              const building1 = greenBuildings[i];
              const building2 = greenBuildings[j];
              
              if (!building1.geometry || !building2.geometry) continue;

              const coords1 = building1.geometry.coordinates[0][0];
              const coords2 = building2.geometry.coordinates[0][0];
              ...
              // Only connect buildings within a certain distance
              if (distance < 0.005) {
                  lines.features.push({
                      type: 'Feature',
                      properties: {
                          intensity: 1 - (distance * 200)
                      },
                      geometry: {
                          type: 'LineString',
