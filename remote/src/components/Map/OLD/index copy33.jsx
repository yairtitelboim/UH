...
const CogentCard = ({ coordinates }) => {
  const iconColor = '#51ff00';
  
  return (
    <div style={{
      position: 'fixed', // Changed from absolute to fixed
      left: coordinates[0],
      top: coordinates[1] - 100, // Offset upward
...
const MapComponent = (props) => {
  ...
  const cogentCardRef = useRef(null);
  const cogentCardContainer = useRef(null);
  ...
  // Add this useEffect to create the container
  useEffect(() => {
    console.log('Creating Cogent card container');
    // Create container for Cogent card if it doesn't exist
    if (!cogentCardContainer.current) {
      ...
      container.id = 'cogent-card-container'; // Add ID for debugging
      document.body.appendChild(container);
      cogentCardContainer.current = container;
      console.log('Cogent card container created');
    }

    return () => {
      if (cogentCardContainer.current) {
        document.body.removeChild(cogentCardContainer.current);
        console.log('Cogent card container removed');
      }
    };
  }, []);

  // Modify handleCogentClick to show the card
  const handleCogentClick = useCallback(() => {
    ...
    try {
      if (!cogentActive) {
        // Get the currently selected building's coordinates
        if (selectedBuilding) {
          const point = map.current.project([
            selectedBuilding.location.longitude,
            selectedBuilding.location.latitude
          ]);

          console.log('Selected building coordinates:', point);

          // Clear existing card
          if (cogentCardContainer.current) {
            ReactDOM.unmountComponentAtNode(cogentCardContainer.current);
          }

          // Render new card
          ReactDOM.render(
            <CogentCard coordinates={[point.x, point.y]} />,
            cogentCardContainer.current
          );
